...
const motivoColor = m=>{
  if(!m)return C.muted;
  if(m.toLowerCase().includes("macellat"))return C.bovini;
  if(m.toLowerCase().includes("mort"))return C.red;
  if(m.toLowerCase().includes("vendut"))return C.green;
  if(m.toLowerCase().includes("furto")||m.toLowerCase().includes("scappat"))return C.yellow;
  return C.muted;
};
const motivoIcon = m=>{
  if(!m)return "📤";
  if(m.toLowerCase().includes("macellat"))return "🔪";
  if(m.toLowerCase().includes("mort"))return "✝️";
  if(m.toLowerCase().includes("vendut"))return "💰";
  if(m.toLowerCase().includes("furto"))return "🚨";
  if(m.toLowerCase().includes("scappat"))return "🏃";
  return "📤";
};
  ...
  return(
    <div style={{padding:"16px 16px 100px"}}>
      ...
      <Card style={{background:specieColor(animale.specie)+"12",
        ...
        <div style={{fontWeight:700,fontSize:16}}>{specieIcon(animale.specie)} {animale.nome||animale.bdn}</div>
      ...
      <Field label="Data uscita" value={form.data_uscita} onChange={v=>setForm(f=>({...f,data_uscita:v}))}
        type="date"/>
      {gg>0&&(
      ...
      {resa&&(
        <div style={{background:C.green+"12",border:`1px solid ${C.green}33`,borderRadius:10,
          padding:"8px 12px",marginBottom:12,fontSize:13}}>
          ⚖️ Resa macellazione: <strong style={{color:C.green}}>{resa}%</strong>
        </div>
      )}
...
export default function RegistroUscite() {
  ...
  const carica=async()=>{
    setLoading(true);
    const{data}=await supabase.from("animali").select("*")
      .order("stato").order("data_uscita",{ascending:false});
    setAnimali(data||[]);
    setLoading(false);
  };
  useEffect(()=>{carica();},[]);
  ...
  if(formUscita) return wrap(
    <FormUscita animale={formUscita} onSave={()=>{setFormUscita(null);carica();}}
            ...
            {subTab==="usciti"&&(
              <>
                ...
                {macellati.length>0&&(
                  <Card style={{background:`linear-gradient(135deg,${C.bovini}15,${C.card})`}}>
                    <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:10}}>
                      🔪 RIEPILOGO MACELLAZIONI ({macellati.length} capi)
                    </div>
                ...
                {usciti.length===0?(
                  <div style={{textAlign:"center",padding:40,color:C.muted}}>
                    <div style={{fontSize:40,marginBottom:8}}>📤</div>
                    <div>Nessuna uscita registrata</div>
                    <div style={{marginTop:8,fontSize:13}}>
                      Vai su "In stalla" e tocca "Uscita" per registrare
                    </div>
                  </div>
                ):usciti.map(a=>{
                  ...
                  return(
                    <Card key={a.id} style={{borderLeft:`4px solid ${col}`}}>
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>
                        <Badge label={specieIcon(a.specie)+" "+specieLabel(a.specie)}
                          color={specieColor(a.specie)}/>
                        {a.motivo_uscita&&<Badge label={a.motivo_uscita} color={col}/>}
                        {a.stato&&a.stato!=="attivo"&&<Badge label={a.stato} color={C.muted}/>}
                      </div>
